...
export default function portfolioReturnCalculator(userInput) {
  ...
  let retirementSalaryValid = userInput.retirementSalary
  ...
  for (
    ...
    if (i >= preRetirementLength) {
      ...
      retirementDraw.push(
        -(
          -1 * retirementDraw[i - 1] * (1 + getNormalizedReturn) +
  ...
  if (xAxisLength - startYear + userInput.birthYear > 0) {
    labels = [...Array(xAxisLength - startYear + userInput.birthYear)].map(
      (_, i) => i + startYear
    );
  }
  return {
