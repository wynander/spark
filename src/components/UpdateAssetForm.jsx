...
export default function UpdateAssetForm({
	...
	if (user) {
		initialValuesIndex = assetValues.findIndex((asset) => asset.dbid === activeAssetId);
	...
	if (activeAssetId === null) {
		return (
			<div className="no-asset-container">
				<Icon className="arrow left no-asset-arrow" size="big"></Icon>
				<h1 className="no-asset-index-msg">Click an Asset to review or update its properties</h1>
			</div>
		);
	} else {
		return (
